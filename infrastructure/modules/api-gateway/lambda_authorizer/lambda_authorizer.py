...
#    try:
    log.debug("Top of try")
    # Set the principalId to the accountId making the authorizer call
    principalId = event['requestContext']['accountId']

    log.debug("principalId: %s", principalId)

    tmp = event['routeArn'].split(':')
    log.debug("tmp: %s", tmp)
    apiGatewayArnTmp = tmp[5].split('/')
    log.debug("apiGatewayArnTmp: %s", apiGatewayArnTmp)
    awsAccountId = tmp[4]
    log.debug("awsAccountId: %s", awsAccountId)

    log.debug("principalId: %s awsAccountId: %s", principalId, awsAccountId)
    ...
    username,  password = base64.standard_b64decode(username_password_hash).decode('UTF-8').split(':')
    log.debug("username: %s password: %s", username, password)


    lambda_payload = json.dumps({"username": username, "password": password})
    response = lambda_client.invoke(
        FunctionName='staging-partner_config',
        InvocationType='RequestResponse',
        Payload=lambda_payload)

    payload = json.load(response['Payload'])
    valid_auth = payload['valid_auth']
    log.debug("response: %s valid_auth: %s", payload, valid_auth)
