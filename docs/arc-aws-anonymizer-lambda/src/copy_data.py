...
def load_data():
    global  addDocumentList, removeDocumentList , addApplicationList, removeApplicationList , addStipVerificationListStip , addStipVerificationList,  removeStipVerificationList, appSkipTheStip, removeSkipTheStip
    # Document List 
    ...
    # Skip the Stip
    appSkipTheStip = os.environ['ADD_SKIP_THE_STIP_PII'].split(',')
    removeSkipTheStip = os.environ['REMOVE_SKIP_THE_STIP_PII'].split(',')
...
def lambda_handler(event, context):
    ...
    if pyrex_sts(source_key):
        ...
        parse_PII(pii_data,appSkipTheStip,removeSkipTheStip)
        uploadByteStream = bytes(json.dumps(pii_data).encode("utf-8"))
        target_key = source_key # Change if desired
        s3_client.put_object(Bucket=TARGET_BUCKET, Key=target_key, Body=uploadByteStream)
